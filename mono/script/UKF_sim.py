...
if __name__ == '__main__':
    ...
    for i in range(N_mc):
        ...
        locationUKF.x = np.array([x_init,0.001,y_init,0.001,z_init,0.001,170*np.pi/180,1*np.pi/180,10*np.pi/180\
                                ,1*np.pi/180,10*np.pi/180,1*np.pi/180,l_init])
        # locationUKF.x = np.array([1.5,0.001,-0.5,0.001,4,0.001,170*np.pi/180,1*np.pi/180,10*np.pi/180\
        #                       ,1*np.pi/180,10*np.pi/180,1*np.pi/180,l_init])
        ...
        for j in range(Steps_max):
            ...
            x_data.append(float(locationUKF.x[0]) - X)
            y_data.append(float(locationUKF.x[2]) - Y)
            z_data.append(float(locationUKF.x[4]) - Z)
            l_data.append(float(locationUKF.x[12]) )
            # 保存数据成pkl
            x_data_pkl.append(float(locationUKF.x[0]) - X)
            y_data_pkl.append(float(locationUKF.x[2]) - Y)
            z_data_pkl.append(float(locationUKF.x[4]) - Z)
            l_data_pkl.append(float(locationUKF.x[12]) - 0.8)
    ...
    # with open('UKF_sin_data1.pkl', 'wb') as f:
    #     pickle.dump(data, f)
    ...
    plt.title('UKF Monte Carlo Estimation Error (100 runs)')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()
