...
def detect(save_img=False):
    ...
    while not rospy.is_shutdown():
        ...
        for i, det in enumerate(pred):
            ...
            # mean filtering
            res_list_x.append(vision_info.logo_pos.x)   
            res_list_y.append(vision_info.logo_pos.y)   
            res_list_z.append(vision_info.logo_pos.z)   

            res_list_x.pop(0)
            res_list_y.pop(0)
            res_list_z.pop(0)

            total_x = sum(res_list_x)
            total_y = sum(res_list_y)
            total_z = sum(res_list_z)

            count_x = len(res_list_x)
            count_y = len(res_list_y)
            count_z = len(res_list_z)

            vision_info_mean = YoloRes()
            vision_info_mean.update_yolo_res = vision_info.update_yolo_res

            vision_info_mean.logo_pos.x = total_x / count_x
            vision_info_mean.logo_pos.y = total_y / count_y
            vision_info_mean.logo_pos.z = total_z / count_z

            vision_info_mean.QR_pos.x = total_x / count_x
            vision_info_mean.QR_pos.y = total_y / count_y
            vision_info_mean.QR_pos.z = total_z / count_z
            # print(count_x)
            vision_info_pub.publish(vision_info_mean)
