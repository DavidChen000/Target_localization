...
def plot_one_text(x, img, color=None, label=None, line_thickness=None):
    # Plots one bounding box on image img
    tl = line_thickness or round(
        0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    c1, c2 = (int(x[0]), int(x[1] - 5)), (int(x[0] + 260), int(x[1] - 25))
    color = [255, 255, 0]
    cv2.rectangle(img, c1, c2, color, thickness=cv2.FILLED)
    c1, c2 = (int(x[0]), int(x[1] - 5)), (int(x[2]), int(x[3] - 5))
    ...
    if type(label) == int:
        ...
        label = label
    cv2.putText(img, str(label), (c1[0], c1[1] - 2), 0,
                tl / 6, [0, 0, 255], thickness=1, lineType=cv2.LINE_AA)
    # cv2.putText(img, str(label), (c2[0], c1[1] - 3), 0, tl / 6, [0, 0, 255], thickness=1, lineType=cv2.LINE_AA)
