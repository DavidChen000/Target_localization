...
class SIYISDK:
    ...
    def requestGimbalAngle(self, yaw_angle:int, pitch_angle:int):
        """
        Sends request for gimbal angle

        Params
        --
        yaw_angle [int]: -1350~0~1350(-135.0°~0°~135.0°): 
        pitch_angle [int]: -900~0~250(-90.0°~0°~25.0°): 
        
        Returns
        --
        [bool] True: success. False: fail
        """
        msg = self._out_msg.gimbalAngleMsg(yaw_angle, pitch_angle)
        if not self.sendMsg(msg):
            return False
        return True
