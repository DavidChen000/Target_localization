...
import logging
from utils import toHex,toTwoBits
...
class SIYIMESSAGE:
    ...
    def gimbalAngleMsg(self, yaw_angle, pitch_angle):
        """
        Gimbal angle control Msg.
        Yaw_angle Values: -1350~0~1350(-135.0°~0°~135.0°): 
        Pitch_angle Values: -900~0~250(-90.0°~0°~25.0°): 
        Negative and positive represent two directions.
        The angular control accuracy is 1(0.1°)
        

        Params
        --
        - yaw_angle [int16_t] in 10*degrees
        - pitch_angle [int16_t] in 10*degrees
        """
        if yaw_angle>1350:
            print("siyi_message.py/gimbalAngleMsg Warning! The yaw_angular limit is 135 degrees!")
            yaw_angle=1350
        if yaw_angle<-1350:
            print("siyi_message.py/gimbalAngleMsg Warning! The yaw_angular limit is -135 degrees!")
            yaw_angle=-1350
        ...
        if pitch_angle>250:
            print("siyi_message.py/gimbalAngleMsg Warning! The pitch_angular limit is 25 degrees!")
            pitch_angle=250
        if pitch_angle<-900:
            print("siyi_message.py/gimbalAngleMsg Warning! The pitch_angular limit is -90 degrees!")
            pitch_angle=-900

        data1=toHex(yaw_angle, 16)
        data1 = toTwoBits(data1)
        # print("yaw_angle:",yaw_angle,data1)

        data2=toHex(pitch_angle, 16)
        data2 = toTwoBits(data2)
        # print("pitch_angle:",pitch_angle,data2)
        
        data=data1+data2
        # print("data:",data)
        cmd_id = COMMAND.GIMBAL_ANGLE
        return self.encodeMsg(data, cmd_id)
